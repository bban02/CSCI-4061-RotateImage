...
void * worker(void *args)
{
    ...
    while (1) {
        pthread_mutex_lock(&lock);
        request_t request = requestQueue[next];
    ...
    // pthread_exit(NULL);
        printf("Qlen %d next %d filName %s\n", qLen, next, request.filename);
        next ++;
        // return NULL;
            // Stbi_load takes: A file name, int pointer for width, height, and bpp


       uint8_t* image_result = stbi_load(request.filename, &width, &height, &bpp,  CHANNEL_NUM);
        ...
        */
        ...
        //img_array
        //width*CHANNEL_NUM
        stbi_write_png(log_file, width, height, CHANNEL_NUM, img_array, width*CHANNEL_NUM);
